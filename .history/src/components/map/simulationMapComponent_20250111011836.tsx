...
const MapComponent: React.FC<MapComponentProps> = ({ trajectories, onPointClick, showHeatmap }) => {
  ...
  useEffect(() => {
    ...
    const mapInstance = new mapboxgl.Map({
      ...
      center: [118.3889, 24.4539], // 注意 Mapbox 使用 [lng, lat] 顺序
      zoom: 13
    });
      ...
      if (showHeatmap) {
          ...
          data: {
            ...
            features: trajectories.flatMap(trajectory =>
              trajectory.points.map(point => ({
                ...
                geometry: {
                  type: 'Point',
                  coordinates: [point[1], point[0]] // 转换为 [lng, lat]
                }
              }))
            )
          }
        ...
        const geojsonData = {
          ...
          features: trajectories.map(trajectory => ({
            ...
            geometry: {
              type: 'LineString',
              coordinates: trajectory.points.map(point => [point[1], point[0]]) // 转换为 [lng, lat]
            }
          }))
        };
