...
const MapComponent: React.FC<MapComponentProps> = ({ 
  ...
  console.log('Received trajectories:', trajectories);
  const mapContainer = useRef<HTMLDivElement>(null);
  const map = useRef<mapboxgl.Map | null>(null);
  const markers = useRef<mapboxgl.Marker[]>([]);

  // 转换轨迹数据格式
  const convertedTrajectories: Trajectory[] = trajectories.map((traj, index) => ({
  ...
  useEffect(() => {
    ...
    try {
      const mapStyle = {
        ...
        sources: {
          'raster-tiles': {
            type: 'raster',
            tiles: [
              'https://api.mapbox.com/styles/v1/mapbox/streets-v12/tiles/{z}/{x}/{y}?access_token=' + mapboxgl.accessToken
            ],
            tileSize: 256
          }
        },
      ...
      mapInstance.on('load', () => {
        if (showHeatmap) {
          addHeatmapLayer(mapInstance, convertedTrajectories);
        } else {
          addTrajectoryLayer(mapInstance, convertedTrajectories);
        }
      });
  ...
  }, [convertedTrajectories, showHeatmap]);
  ...
  useEffect(() => {
    ...
    try {
      ...
      // 更新图层
      if (showHeatmap) {
        updateHeatmapLayer(map.current, convertedTrajectories);
      } else {
        updateTrajectoryLayer(map.current, convertedTrajectories);
      }
    } catch (error) {
