...
function getTrajectoryExplanation(scenario: string, event: string, temperature: number): TrajectoryExplanation {
  const explanations = {
    normal: {
      base: '在普通日期，游客轨迹呈现较为分散的特点，主要集中在景区核心区域。',
      high_temp: '由于较高的温度值，生成的轨迹展现出更多样化的游览路线。',
      low_temp: '由于较低的温度值，生成的轨迹较为保守，主要沿着热门路线。'
    },
    rainy: {
      base: '在雨天情况下，游客轨迹倾向于选择有遮蔽的路线，停留点较为集中。',
      high_temp: '即使在雨天，较高的温度值仍然产生了一些创新的避雨路线。',
      low_temp: '在雨天和低温度值的双重影响下，轨迹高度集中在有遮蔽的区域。'
    },
    peak: {
      base: '在旺季期间，游客轨迹呈现出较高的密度，各景点间的连接更为频繁。',
      high_temp: '旺季的高温度值产生了更多非常规路线，体现出游客寻找替代路线的倾向。',
      low_temp: '即使在旺季，低温度值仍然产生了相对保守的路线选择。'
    }
  };

  const eventExplanations = {
    music_festival: '音乐节的举办使得钢琴博物馆周边区域出现明显的人流聚集。',
    food_festival: '美食节的影响使得龙头路区域的停留时间明显延长。',
    light_show: '灯光秀活动导致鼓浪屿码头区域在夜间出现显著的人流密集。'
  };
